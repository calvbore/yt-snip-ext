...
(function (root, factory) {
   ...
   * declared `get(keys, callback)` / `set(items, callback)` → arity 2;
   * Firefox's promise-based ones are `get(keys)` / `set(items)` → arity 1.
   * Consulted per method, so a store that only exposes one of them still
   * resolves the right style for the call being made.
   * Returns 'callback' or 'promise'.
   */
  function detectStyle(store) {
    if (store && store.set) {
      return store.set.length >= 2 ? 'callback' : 'promise';
    }
    if (store && store.get) {
